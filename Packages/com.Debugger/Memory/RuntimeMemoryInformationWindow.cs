...
namespace ZeroFramework.Debugger
{
        ...
        where T : UnityEngine.Object
    {
                ...
                else
                {
                    if (_duplicateSimpleCount > 0)
                    {
                        GUILayout.Label(Utility.Text.Format(
                            ...
                            _samples.Count, typeName, GetByteLengthString(_sampleSize), _sampleTime.ToLocalTime(),
                            _duplicateSimpleCount, GetByteLengthString(_duplicateSampleSize)));
                    }
                    else
                    {
                        GUILayout.Label(Utility.Text.Format(
                            "<b>{0} {1}s ({2}) obtained at {3:yyyy-MM-dd HH:mm:ss}.</b>", _samples.Count, typeName,
                            GetByteLengthString(_sampleSize), _sampleTime.ToLocalTime()));
                    }
                    ...
                    for (int i = 0; i < _samples.Count; i++)
                    {
                        GUILayout.BeginHorizontal();
                        {
                            GUILayout.Label(_samples[i].Highlight
                                ? Utility.Text.Format("<color=yellow>{0}</color>", _samples[i].Name)
                                : _samples[i].Name);
                            GUILayout.Label(
                                _samples[i].Highlight
                                    ? Utility.Text.Format("<color=yellow>{0}</color>", _samples[i].Type)
                                    : _samples[i].Type, GUILayout.Width(240f));
                            GUILayout.Label(
                                _samples[i].Highlight
                                    ? Utility.Text.Format("<color=yellow>{0}</color>",
                                        GetByteLengthString(_samples[i].Size))
                                    : GetByteLengthString(_samples[i].Size), GUILayout.Width(80f));
                        }
                        GUILayout.EndHorizontal();
        ...
        private void TakeSample()
        {
            ...
            {
                if (_samples[i].Name == _samples[i - 1].Name && _samples[i].Type == _samples[i - 1].Type &&
                    _samples[i].Size == _samples[i - 1].Size)
                {
