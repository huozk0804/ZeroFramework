...
namespace ZeroFramework.FileSystem
{
    ...
    public abstract class FileSystemStream
    {
        ...
        /// 缓存二进制流的长度。
        /// </summary>
        internal const int CachedBytesLength = 0x1000;

        /// <summary>
        /// 缓存二进制流。
        /// </summary>
        internal static readonly byte[] s_CachedBytes = new byte[CachedBytesLength];
        ...
        internal abstract void SetLength(long length);
        ...
        internal abstract void Seek(long offset, SeekOrigin origin);
        ...
        internal abstract int Read(byte[] buffer, int startIndex, int length);
        ...
        /// <param name="length">存储读取文件内容的二进制流的长度。</param>
        /// <returns>实际读取了多少字节。</returns>
        internal int Read(Stream stream, int length)
        {
            int bytesRead = 0;
        ...
        internal abstract void Write(byte[] buffer, int startIndex, int length);
