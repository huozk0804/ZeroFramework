...
namespace ZeroFramework.DataTable
{
    ...
    public sealed partial class DataTableManager : GameFrameworkModule, IDataTableManager
    {
        ...
        public DataTableManager()
        {
            ...
            var helper = Helper.CreateHelper(GameFrameworkConfig.Instance.m_DataTableHelperTypeName,
                GameFrameworkConfig.Instance.m_CustomDataTableHelper);
            SetDataProviderHelper(helper);
            SetDataTableHelper(helper);

            var size = GameFrameworkConfig.Instance.m_DataTableCachedBytesSize;
            if (size > 0)
            {
        ...
        {
            if (m_DataTables.TryGetValue(typeNamePair, out var dataTable))
            {
                return dataTable;
            }
